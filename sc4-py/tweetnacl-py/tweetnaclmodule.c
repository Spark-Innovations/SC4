...
PyObject *pycrypto_sign_keypair_from_seed(PyObject *self,
					  PyObject *args, PyObject *kw) {

    const char *seed;
    Py_ssize_t seedsize = 0;
    static const char *kwlist[] = {"seed", 0};
    PyObject *pypk, *pysk, *ret;

    if (!PyArg_ParseTupleAndKeywords(args, kw,
                                     "|s#:crypto_sign_keypair_from_seed",
                                     (char **)kwlist,
                                     (char **)&seed, &seedsize)) {
        return (PyObject *)0;
    }

    if (seedsize != crypto_sign_SECRETKEYBYTES) {
      return naclexception("incorrect seed length");
    }

    pypk = PyBytes_FromStringAndSize((char *)0, crypto_sign_PUBLICKEYBYTES);
    if (!pypk) return (PyObject *)0;
    pysk = PyBytes_FromStringAndSize(seed, crypto_sign_SECRETKEYBYTES);
    if (!pysk) {
        Py_DECREF(pypk);
        return (PyObject *)0;
    }
...
PyObject *pyspk2epk(PyObject *self, PyObject *args, PyObject *kw) {

    const char *spk;
    Py_ssize_t spksize = 0;
    static const char *kwlist[] = {"spk", 0};
    PyObject *pyepk, *pyspk;
    ...
    if (spksize != crypto_sign_PUBLICKEYBYTES) {
      return naclexception("incorrect public key length");
    }

    pyepk = PyBytes_FromStringAndSize((char *)0, crypto_box_PUBLICKEYBYTES);
    if (!pyepk) return (PyObject *)0;
    pyspk = PyBytes_FromStringAndSize(spk, crypto_sign_PUBLICKEYBYTES);
    if (!pyspk) {
        Py_DECREF(pyepk);
        return (PyObject *)0;
    }
    ...
    spk2epk(
        (unsigned char *)PyBytes_AS_STRING(pyepk),
        (unsigned char *)PyBytes_AS_STRING(pyspk)
    );
...
static PyMethodDef nacl_methods[] = {
    ...
    {"crypto_sign_keypair_from_seed", (PyCFunction)pycrypto_sign_keypair_from_seed, METH_VARARGS | METH_KEYWORDS, pycrypto_sign_keypair_from_seed__doc__},
    ...
    {"spk2epk", (PyCFunction)pyspk2epk, METH_VARARGS | METH_KEYWORDS, pyspk2epk__doc__},
    {(void *)0, (void *)0}
};
...
#endif
    PyObject *m;
    unsigned char dummy[1];  // Is this really necessary???
#if PY_MAJOR_VERSION >= 3
