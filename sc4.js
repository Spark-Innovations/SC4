...
(function() {
  ...
  function decrypt_multi(bytes) {
	...
	var sdr = b58(sender_key).slice(0,8) + '... for multiple recipients';
	return (msg ? [msg, b58(sender_key), sdr] : null);
      }
    }
    return null;
  }

  function encrypt_multi_pt(thing, recipient_list) {
    return split_into_lines(b64(encrypt_multi(thing, recipient_list)));
  }

  function decrypt_multi_pt(s) {
    try {
      return decrypt_multi(unb64(s.split('\n').join('')));
    } catch (e) {
      return null;
    }
  }
  ...
  function sc4_typeof(thing) {
    if (typeof thing == 'string') {
      ...
      if (enc_pt_multi_regex.test(thing)) return "multi_enc_pt";
      return null;
      ...
      if (nacl.verify(hdr, multi_enc_header)) return 'multi_enc';
    }
    return null;
  }
  ...
  sc4.exports = [
    ...
    encrypt_multi, decrypt_multi, encrypt_multi_pt, decrypt_multi_pt,
